# Source: moved from mnist_dataset.py (rewrite-minimal: updated imports only)
# Custom MNIST loader that preserves the mnist20x20 border-removal behavior.
# MNISTRemoveBorderTransform must be kept — it is required for mnist20x20 semantics.
import codecs
import os
import os.path
import warnings
...
class MNISTRemoveBorderTransform:
    """Crop blank border rows/columns from MNIST images to produce 20x20 crops."""
...
class MNIST(VisionDataset):
    """`MNIST <http://yann.lecun.com/exdb/mnist/>`_ Dataset with optional border removal."""
...
# ── Low-level MNIST binary file readers ─────────────────────────────────────
