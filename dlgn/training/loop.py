...
from dlgn.data.loaders import load_dataset, cycle_loader, input_dim_of_dataset, num_classes_of_dataset
...
def train_model(
    ...
    # Pass threshold_bits so cifar10-family datasets compute their effective
    # input dim from the override rather than the static registry entry (D15).
    input_dim = input_dim_of_dataset(config['dataset'], config.get('threshold_bits'))
