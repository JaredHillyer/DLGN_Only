...
def run_deep_conv_path(args, train_loader, test_loader, class_count, on_eval=None):
    """Multi-block conv + variable-width FC stack.

    on_eval, when provided, is called at every eval point as
    on_eval(step, train_loss, metrics, elapsed_s) — step 0 is the pre-training
    baseline (train_loss=None). The sweep uses it to stream a per-eval trace.
    Returns a result dict: before/final metrics + compile/total time + flatten_dim.
    """
    from dlgn.data.loaders import cycle_loader
    ...
    if args.train_steps == 1:
        ...
        print(f'  step {1:>5}/{args.train_steps}  '
              ...
              f'[0s]', flush=True)
        if on_eval is not None:
            on_eval(1, float(loss), metrics, time.time() - t_train)
    ...
    for step in range(2, args.train_steps + 1):
        ...
        if step % eval_every == 0 or step == args.train_steps:
            ...
            if on_eval is not None:
                on_eval(step, float(loss), metrics, elapsed)
...
PRESET_GROUPS = {
    'one_tau': PRESETS_ONE_TAU,
    'scaled_tau': PRESETS_SCALED_TAU,
}

# Both groups define the same 12 preset keys.
SWEEP_GROUP_NAMES = ['one_tau', 'scaled_tau']
SWEEP_PRESET_KEYS = sorted(set(PRESETS_ONE_TAU) | set(PRESETS_SCALED_TAU))
SWEEP_POOL_TYPES = ['or', 'max']

# Held fixed for every run in --sweep mode.
SWEEP_REMAT = 'both'
SWEEP_REMAT_POLICY = 'nothing'

# CSV schemas for the sweep outputs.
SUMMARY_FIELDS = [
    'group', 'preset', 'pool_type', 'dataset', 'batch_size', 'threshold_bits',
    'sum_tau', 'learning_rate', 'weight_decay', 'clip_value', 'train_steps',
    'eval_every', 'seed', 'logic_family', 'architecture', 'remat', 'remat_policy',
    'conv_block_channels', 'conv_block_kernel_sizes', 'conv_block_depths',
    'conv_block_paddings', 'conv_block_strides', 'pool_size', 'pool_stride',
    'fc_sizes', 'flatten_dim',
    'before_soft_loss', 'before_hard_loss', 'before_soft_acc', 'before_hard_acc',
    'before_soft_hard_gap', 'final_soft_loss', 'final_hard_loss', 'final_soft_acc',
    'final_hard_acc', 'final_soft_hard_gap',
    'compile_time_s', 'total_time_s', 'wall_s', 'status', 'error',
]

TRACE_FIELDS = [
    'group', 'preset', 'pool_type', 'step', 'train_loss',
    'soft_loss', 'hard_loss', 'soft_acc', 'hard_acc', 'soft_hard_gap', 'elapsed_s',
]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--preset', default='small', choices=SWEEP_PRESET_KEYS)
    p.add_argument('--tau-group', default='one_tau',
                   choices=SWEEP_GROUP_NAMES, dest='tau_group',
                   help='Which preset group --preset draws from in single-run mode '
                        '(ignored by --sweep, which runs both groups).')
    ...
    g.add_argument('--sweep', action='store_true', dest='sweep',
                   help='Run the full preset sweep: both tau groups x all presets x '
                        'both pool types, with remat=both, remat-policy=nothing, and the '
                        'JAX compile cache enabled. Writes summary + trace CSVs.')
    g.add_argument('--summary-csv', default=None, dest='summary_csv',
                   help='Sweep summary CSV (one row per run). Default: '
                        'outputs/mnist_layers_sweep_summary_<timestamp>.csv')
    g.add_argument('--trace-csv', default=None, dest='trace_csv',
                   help='Sweep trace CSV (one row per eval point). Default: '
                        'outputs/mnist_layers_sweep_trace_<timestamp>.csv')
    g.add_argument('--sweep-groups', nargs='+', default=list(SWEEP_GROUP_NAMES),
                   choices=SWEEP_GROUP_NAMES, dest='sweep_groups',
                   help='Subset of tau groups to sweep (default: both).')
    g.add_argument('--sweep-presets', nargs='+', default=None, dest='sweep_presets',
                   help='Subset of preset names to sweep (default: all 12).')
    g.add_argument('--sweep-pool-types', nargs='+', default=list(SWEEP_POOL_TYPES),
                   choices=SWEEP_POOL_TYPES, dest='sweep_pool_types',
                   help='Subset of pool types to sweep (default: or max).')
    g.add_argument('--sweep-train-steps', type=int, default=None,
                   dest='sweep_train_steps',
                   help='Override train_steps for every sweep run (default: use each '
                        "preset's own value). Handy for smoke tests.")
    return p


def apply_preset(args, cli_argv):
    """Apply preset defaults for any field not explicitly set on the CLI."""
    group = PRESET_GROUPS.get(args.tau_group, {})
    preset = group.get(args.preset, {})
...
# ---------------------------------------------------------------------------
# Sweep: all presets (both tau groups) x both pool types -> CSV
# ---------------------------------------------------------------------------

def _make_run_args(base_args, preset, pool_type, train_steps_override=None):
    """Clone base args, apply a preset dict, and pin the fixed sweep settings."""
    run_args = copy.deepcopy(base_args)
    for attr, value in preset.items():
        setattr(run_args, attr, value)
    run_args.pool_type = pool_type
    run_args.remat = SWEEP_REMAT
    run_args.remat_policy = SWEEP_REMAT_POLICY
    if getattr(run_args, 'dataset', None) is None:
        run_args.dataset = 'mnist'
    if train_steps_override is not None:
        run_args.train_steps = train_steps_override
    return run_args


def _build_loaders(args, cache):
    """Return (train_loader, test_loader, class_count), memoized by
    (dataset, batch_size, threshold_bits) so identical loaders aren't rebuilt."""
    from dlgn.data.loaders import load_dataset, num_classes_of_dataset

    tb = getattr(args, 'threshold_bits', None)
    key = (args.dataset, args.batch_size, tb)
    if key not in cache:
        ...
        if tb is not None:
            data_config['threshold_bits'] = tb
        train_loader, _, test_loader = load_dataset(data_config)
        cache[key] = (train_loader, test_loader,
                      num_classes_of_dataset(args.dataset))
    return cache[key]


def _summary_row(group_name, preset_name, pool_type, run_args,
                 result, wall, status, error):
    """Assemble one summary.csv row from a run's config + result dict."""
    row = {f: '' for f in SUMMARY_FIELDS}
    row.update({
        'group': group_name,
        'preset': preset_name,
        'pool_type': pool_type,
        'dataset': getattr(run_args, 'dataset', 'mnist'),
        'batch_size': getattr(run_args, 'batch_size', ''),
        'threshold_bits': getattr(run_args, 'threshold_bits', ''),
        'sum_tau': getattr(run_args, 'sum_tau', ''),
        'learning_rate': getattr(run_args, 'learning_rate', ''),
        'weight_decay': getattr(run_args, 'weight_decay', ''),
        'clip_value': getattr(run_args, 'clip_value', ''),
        'train_steps': getattr(run_args, 'train_steps', ''),
        'eval_every': getattr(run_args, 'eval_every', ''),
        'seed': getattr(run_args, 'seed', ''),
        'logic_family': getattr(run_args, 'logic_family', ''),
        'architecture': getattr(run_args, 'architecture', ''),
        'remat': getattr(run_args, 'remat', ''),
        'remat_policy': getattr(run_args, 'remat_policy', ''),
        'conv_block_channels': json.dumps(list(getattr(run_args, 'conv_block_channels', []))),
        'conv_block_kernel_sizes': json.dumps(list(getattr(run_args, 'conv_block_kernel_sizes', []))),
        'conv_block_depths': json.dumps(list(getattr(run_args, 'conv_block_depths', []))),
        'conv_block_paddings': json.dumps(list(getattr(run_args, 'conv_block_paddings', []))),
        'conv_block_strides': json.dumps(list(getattr(run_args, 'conv_block_strides', []))),
        'pool_size': getattr(run_args, 'pool_size', ''),
        'pool_stride': getattr(run_args, 'pool_stride', ''),
        'fc_sizes': json.dumps(list(getattr(run_args, 'fc_sizes', []))),
        'wall_s': round(wall, 3),
        'status': status,
        'error': error,
    })
    if result is not None:
        b, f = result['before'], result['final']
        row.update({
            'flatten_dim': result.get('flatten_dim', ''),
            'compile_time_s': round(result.get('compile_time_s', 0.0), 3),
            'total_time_s': round(result.get('total_time_s', 0.0), 3),
            'before_soft_loss': b['soft_loss'], 'before_hard_loss': b['hard_loss'],
            'before_soft_acc': b['soft_acc'], 'before_hard_acc': b['hard_acc'],
            'before_soft_hard_gap': b['soft_hard_gap'],
            'final_soft_loss': f['soft_loss'], 'final_hard_loss': f['hard_loss'],
            'final_soft_acc': f['soft_acc'], 'final_hard_acc': f['hard_acc'],
            'final_soft_hard_gap': f['soft_hard_gap'],
        })
    return row


def run_sweep(args) -> int:
    """Run every selected preset x pool type, streaming summary + trace CSVs."""
    from dlgn.utils import setup_jit_cache

    # "Jax_Cache store true": force the persistent compile cache on for the sweep.
    setup_jit_cache(args.jit_cache_dir, enabled=True)

    groups = args.sweep_groups
    pool_types = args.sweep_pool_types
    preset_subset = args.sweep_presets  # None -> every key in each group

    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    out_dir = ROOT / 'outputs'
    summary_path = (Path(args.summary_csv) if args.summary_csv
                    else out_dir / f'mnist_layers_sweep_summary_{ts}.csv')
    trace_path = (Path(args.trace_csv) if args.trace_csv
                  else out_dir / f'mnist_layers_sweep_trace_{ts}.csv')
    summary_path.parent.mkdir(parents=True, exist_ok=True)
    trace_path.parent.mkdir(parents=True, exist_ok=True)

    # Build the run matrix up front (group -> preset -> pool_type) for a count.
    matrix = []
    for group_name in groups:
        group = PRESET_GROUPS[group_name]
        names = preset_subset if preset_subset is not None else list(group.keys())
        for preset_name in names:
            if preset_name not in group:
                print(f'  [skip] {group_name}/{preset_name}: not in group')
                continue
            for pool_type in pool_types:
                matrix.append((group_name, preset_name, pool_type))
    total = len(matrix)

    print(f'\n=== SWEEP: {total} runs ('
          f'{len(groups)} groups x presets x {len(pool_types)} pool types) ===')
    print(f'  fixed: remat={SWEEP_REMAT} remat_policy={SWEEP_REMAT_POLICY} jit_cache=on')
    ...
    with open(summary_path, 'w', newline='') as sf, \
            open(trace_path, 'w', newline='') as tf:
        sw = csv.DictWriter(sf, fieldnames=SUMMARY_FIELDS)
        tw = csv.DictWriter(tf, fieldnames=TRACE_FIELDS)
        sw.writeheader(); sf.flush()
        tw.writeheader(); tf.flush()

        for idx, (group_name, preset_name, pool_type) in enumerate(matrix, 1):
            preset = PRESET_GROUPS[group_name][preset_name]
            run_args = _make_run_args(args, preset, pool_type,
                                      train_steps_override=args.sweep_train_steps)
            print(f'\n----- [{idx}/{total}] {group_name}/{preset_name} '
                  f'pool={pool_type} sum_tau={getattr(run_args, "sum_tau", None)} '
                  f'-----', flush=True)

            def on_eval(step, train_loss, metrics, elapsed_s,
                        _g=group_name, _p=preset_name, _pt=pool_type):
                tw.writerow({
                    'group': _g, 'preset': _p, 'pool_type': _pt, 'step': step,
                    'train_loss': '' if train_loss is None else round(float(train_loss), 6),
                    'soft_loss': metrics['soft_loss'], 'hard_loss': metrics['hard_loss'],
                    'soft_acc': metrics['soft_acc'], 'hard_acc': metrics['hard_acc'],
                    'soft_hard_gap': metrics['soft_hard_gap'],
            ...
            status, error = 'ok', ''
            try:
                train_loader, test_loader, class_count = _build_loaders(run_args, loader_cache)
                result = run_deep_conv_path(run_args, train_loader, test_loader,
                                            class_count, on_eval=on_eval)
            except Exception as e:  # keep the sweep going on OOM / bad config
                status, error = 'error', repr(e)[:500]
                print(f'  [error] {group_name}/{preset_name} pool={pool_type}: '
                      f'{error}', flush=True)
            wall = time.time() - t0

            sw.writerow(_summary_row(group_name, preset_name, pool_type, run_args,
                                     result, wall, status, error))
            sf.flush()
            if status == 'ok':
                n_ok += 1
    ...
    print(f'\n=== SWEEP DONE: {n_ok} ok, {n_err} error / {total} ===')
    print(f'  summary: {summary_path}')
