# Source: moved from uci_datasets.py (rewrite-minimal)
# UCI dataset implementations.
# BUG FIX: AdultDataset hours-per-week bucket assignments used `==` instead of `=`
# (lines 193-204 of original uci_datasets.py). Fixed here with a guard comment.
# A regression test in tests/test_adult_dataset.py guards this fix.
# IrisDataset is included for completeness but is NOT in the CLI registry.
...
class AdultDataset(UCIDataset):
    ...
    def preprocess_adult_to_binary_data(data_file_name):
        ...
        continuous_attributes = {
            ...
            'capital-loss': ['no-loss', 'small-loss', 'large-loss'],
            ...
            for i, attr_type in enumerate(attributes):
                ...
                if attr_type in continuous_attributes:
                    ...
                    if attr_type == 'age':
                        ...
                        # BUG FIX: original uci_datasets.py used `==` instead of `=`
                        # (comparison instead of assignment), making attr_v always unset.
                        # All branches below use `=` (assignment). See test_adult_dataset.py.
                        if attr_value == 0:
                            attr_v = 'no-hours'
                        elif attr_value <= 12:
                            attr_v = 'mini-hours'
...
class IrisDataset(UCIDataset):
    """Iris dataset — present for completeness but NOT in the CLI dataset registry."""
    file_list = [
